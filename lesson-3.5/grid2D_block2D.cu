 ...
 * grid2D_block2D.cu
 *
 * Example: 2D grid and 2D block matrix addition using CUDA.
 * - Computes C = A + B for integer matrices.
 * - Demonstrates `dim3` grid/block, device memory transfers, and kernel launch.
 *
 * Build:
 *   nvcc grid2D_block2D.cu -o grid2D_block2D
 *
 * Run:
 *   ./grid2D_block2D
 *
...
int main(void)
{
    ...
    addMatrix<<<block, grid>>>(d_A, d_B, d_C, nx, ny);
